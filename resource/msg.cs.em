...
cs_namespace = 'SIGVerse.RosBridge.%s.'%(package_name)

}@
// Generated by rosidl_generator_cs from @(package_name)/@(interface_path)
    ...
    for member in message.structure.members:
        ...
        if isinstance(type_, NamespacedType):
            ...
            if not typename in included:
                print('using SIGVerse.RosBridge.%s;'%(typename))
...
for message in content.get_elements_of_type(Message):
    ...
    for member in message.structure.members:
        ...
        print(indent * (indent_depth+1) + 'this.%s = %s;'%(member.name, initializer))
    print(indent * indent_depth + '}\n')
    if len(message.structure.members) > 0:
        sys.stdout.write(indent * indent_depth + 'public %s('%message.structure.namespaced_type.name)
        for member in message.structure.members:
            sys.stdout.write('%s %s'%(rosidl_generator_cs.msg_type_to_cs(member.type), member.name))
            if message.structure.members.index(member) < len(message.structure.members)-1:
                sys.stdout.write(', ')
        sys.stdout.write(')\n')
        print(indent * indent_depth + '{')
        for member in message.structure.members:
            print(indent * (indent_depth+1) + 'this.%s = %s;'%(member.name, member.name))
        print(indent * indent_depth + '}\n')
    ...
    indent_depth = indent_depth - 1
    print(indent * indent_depth + '} // class %s'%message.structure.namespaced_type.name)
...
for service in content.get_elements_of_type(Service):
    ...
    print(indent * indent_depth + '{')
    print(indent * (indent_depth+1) + 'base.type = "%s/%s";'%("/".join(service.namespaced_type.namespaces), service.namespaced_type.name))
    print(indent * indent_depth + '}\n')
    ...
    if service.response_message:
        ...
        indent_depth = indent_depth - 1
        print(indent * indent_depth + '} // class %s'%service.response_message.structure.namespaced_type.name)
